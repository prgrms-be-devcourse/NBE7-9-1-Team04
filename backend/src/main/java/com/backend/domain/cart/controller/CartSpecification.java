...
public interface CartSpecification {
    ...
    ResponseEntity<ApiResponse<CartResponse>> addCartItem(@RequestBody CartAddRequest request);

    @Operation(summary = "장바구니 메뉴 수량 변경", description = "특정 메뉴의 수량을 변경합니다.")
    ResponseEntity<ApiResponse<CartResponse>> updateCartItemQuantity (@PathVariable Long menuId, @RequestBody CartUpdateRequest request);

    @Operation(summary = "장바구니 메뉴 삭제", description = "장바구니에서 특정 메뉴를 삭제합니다.")
    ResponseEntity<ApiResponse<Void>> deleteCartItem(@PathVariable Long menuId);

    @Operation(summary = "장바구니 전체 조회", description = "현재 사용자의 장바구니 전체 목록을 조회합니다.")
    ResponseEntity<ApiResponse<CartListResponse>> getCart();
